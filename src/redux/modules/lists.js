...
export const actions = {
    ...
    fetchSuccess: (labelList, items, projects, collaborators) => ({'type': types.FETCH_SUCCESSFUL, payload: {labelList, items, projects, collaborators}}),
    ...
    updateDueDateFilter: (startDate, endDate) => ({type: types.UPDATE_DUE_DATE_FILTER, payload: {startDate, endDate}}),
    updatePriorityFilter: (filteredPriorities) => ({type: types.UPDATE_PRIORITY_FILTER, payload: {filteredPriorities}}),
...
function updateId(state, action) {
    ...
    switch (type) {
        case Item: return updateItemId(state, old_id, new_id);
        case List: return updateListId(state, old_id, new_id);
        default:
...
function updateItemId(state, old_id, new_id) {
    ...
    const { list, item } =  state.lists.reduce((listAndItem, itemList) => {
...
function fetchSuccess(state, action) {
    ...
    // Create Lists.
    const loadedLists = ImmutableList(labelList.map((label) => {
        return new List({
            id: label.id,
            title:label.name,
            items: ImmutableList(
...
function setSortBy(state, action) {
    ...
    if (SORT_BY[field] && SORT_BY_DIRECTION[direction]) {
        const sortBy = state.sortBy.set('field', SORT_BY[field])
            .set('direction', SORT_BY_DIRECTION[direction]);
        return {...state, sortBy}
    } else {
...
export const reducer = (state = initialState, action) => {
    switch (action.type) {
        case types.ADD_NEW_LIST: return addList(state, action);
        case types.RENAME_LIST: return renameList(state, action);
        case types.DELETE_LIST: return deleteList(state, action);
        case types.COMPLETE_LIST: return completeList(state, action);
        case types.REORDER_LIST: return reorderList(state, action);
        case types.ADD_LIST_ITEM: return sortLists(addListItem(state, action));
        case types.MOVE_TO_LIST: return sortLists(moveToList(state, action));
        case types.UPDATE_LIST_ITEM: return updateListItem(state, action);
        case types.UPDATE_ID: return sortLists(updateId(state, action));
        case types.COMPLETE_LIST_ITEM: return completeListItem(state, action);

        case types.UPDATE_LISTS_FILTER: return updateListsFilter(state, action);
        case types.UPDATE_PROJECTS_FILTER: return updateProjectsFilter(state, action);
        case types.UPDATE_DUE_DATE_FILTER: return updateDueDateFilter(state, action);
        case types.UPDATE_PRIORITY_FILTER: return updatePriorityFilter(state, action);
        case types.TOGGLE_ASSIGNEE_FILTER: return toggleAssigneeFilter(state, action);
        case types.SET_NAMED_FILTER: return setNamedFilter(state, action);

        case types.FETCH_REQUEST_SENT: return fetchRequest(state, action);
        case types.FETCH_SUCCESSFUL: return sortLists(fetchSuccess(state, action));
        case types.FETCH_FAILURE: return fetchFailure(state, action);
