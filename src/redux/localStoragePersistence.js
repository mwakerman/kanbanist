...
export function load() {
    ...
    if (localStorage[LOCAL_STORAGE_NAMESPACE]) {
        ...
        // Lists
        loadedState.lists = {};
        loadedState.lists.lists = Immutable.List(jsState.lists.lists.map((listObject) => {
        ...
        const {lists, projects, priorities, start, end, assigned, filter } = 
            Immutable.List(['lists','projects', 'priorities', 'start', 'end', 'assigned', 'filter'])
                .reduce((params, param) => {
        ...
        if (lists || projects || priorities || start || end || assigned || filter) {
            // lists
            filteredListIds = lists ? loadedState.lists.lists.filter(el => lists.indexOf(el.title) < 0).map(el => el.id) :
                Immutable.List.of();

            filteredProjectIds = projects ? loadedState.lists.projects.filter(el => projects.indexOf(el.name) < 0).map(el => el.id) :
                Immutable.List.of();

            filteredPriorities = priorities ? Priorities.filter(el => priorities.indexOf(el.id) < 0) :
                Immutable.List.of();
        ...
        loadedState.lists.filteredProjects = loadedState.lists.projects.filter(project => filteredProjectIds.contains(project.id));
