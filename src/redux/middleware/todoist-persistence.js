    ...
    switch(action.type) {
        ...
        case types.ADD_LIST_ITEM:
            function persistAddListItem() {
                ...
                const projectString = hasProjectSyntax || !defaultProject ? '' : `#${defaultProject.name.replaceAll(' ','')}`;
        ...
        case types.ADD_NEW_LIST:
            function persistAddLabel() {
                ...
                Todoist.addLabel(token, newLabel, temp_id)
                    .then(response => {
                        store.dispatch({type: types.UPDATE_ID, payload: {
                            type: List,
                            old_id: temp_id,
