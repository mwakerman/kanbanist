...
export default class Todoist {
    ...
    static fetch(apiToken) {
        return fetch(`${SYNC_API_URL}?token=${apiToken}&sync_token=*&resource_types=["labels","items","projects","collaborators"]`)
            .then(res => res.json())
            .then(todoistData => {
                // Labels
    ...
    static updateLabelOrder(apiToken, labelOrdering) {
        return Todoist.sendCommand(apiToken, Todoist.createCommand("label_update_orders", {"id_order_mapping": labelOrdering}));
    }

    static updateLabelName(apiToken, labelId, newLabelName) {
        return Todoist.sendCommand(apiToken, Todoist.createCommand("label_update", {id: labelId, name: newLabelName}));
    }
    ...
    static sendCommands(apiToken, commands = []) {
        ...
        return fetch(url).then(resp => resp.json()).catch(err => console.error("Error sending commands: ", err));
    }
}
